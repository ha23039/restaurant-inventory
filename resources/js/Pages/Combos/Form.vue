...
const isEditMode = computed(() => !!props.combo);

// Form state
const form = ref({
    name: props.combo?.name || '',
    description: props.combo?.description || '',
    base_price: props.combo?.base_price || '',
    category: props.combo?.category || 'Combos',
    is_available: props.combo?.is_available ?? true,
    show_in_menu: props.combo?.show_in_menu ?? true,
    show_in_pos: props.combo?.show_in_pos ?? true,
    image: null,
    remove_image: false,
    components: [],
});

const imagePreview = ref(props.combo?.image_path || null);
const processing = ref(false);

// Inicializar componentes si estamos editando
if (props.combo?.components) {
    form.value.components = props.combo.components.map(comp => ({
        id: comp.id,
...
const getProductName = (type, id) => {
    const product = allProducts.value.find(p => p.type === type && p.id === parseInt(id));
    return product?.name || 'Seleccionar producto';
};
...
<template>
    <Head :title="isEditMode ? 'Editar Combo' : 'Nuevo Combo'" />

    <AdminLayout>
        <template #header>
            <div class="flex items-center gap-4">
                <button
                    @click="$inertia.visit(route('combos.index'))"
                ...
                <form @submit.prevent="handleSubmit" class="space-y-6">
                    ...
                    <div class="flex items-center justify-end gap-3">
                        <button
                            type="button"
                            @click="$inertia.visit(route('combos.index'))"
                            class="px-6 py-2.5 text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                        >
                            Cancelar
                        </button>
                        <button
                            type="submit"
                            :disabled="processing"
                            class="px-6 py-2.5 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-400 text-white font-medium rounded-lg transition-colors flex items-center gap-2"
                        >
