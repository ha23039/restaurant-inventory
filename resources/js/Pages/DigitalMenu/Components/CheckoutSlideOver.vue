                    ...
                    <div v-if="deliveryMethod === 'dine_in'">
                        ...
                        >
                            ...
                            <option
                                ...
                                :value="table.id"
                            >
                                Mesa {{ table.table_number }} ({{ table.capacity }} personas){{ table.pending_orders > 0 ? ` - ${table.pending_orders} pedido(s) activo(s)` : '' }}
                            </option>
                        </select>
                        <p v-if="deliveryMethod === 'dine_in' && !selectedTable" class="mt-1 text-xs text-red-500">
                            Debes seleccionar una mesa para continuar
                        </p>
                        <p v-if="selectedTable && availableTables.find(t => t.id === selectedTable)?.pending_orders > 0" class="mt-1 text-xs text-blue-600 dark:text-blue-400 flex items-center">
                            <svg class="w-4 h-4 mr-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" />
                            </svg>
                            Esta mesa ya tiene pedidos. Tu pedido se agregará a la cuenta de la mesa.
                        </p>
                    </div>
